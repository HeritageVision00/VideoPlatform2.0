...
@Component({
  ...
  styleUrls: ['./admin.component.scss', '../graphs/smart-table.scss']
})
export class AdminComponent implements OnInit {
  ...
  hola3(event): void{
    ...
    const windowRef = this.windowService.open(DeleteWinComponent, { title: `Do you want to delete ${event.data.username}'s user?`, context: { type: 5, data: { id: event.data.id, role: event.data.role }}, buttons:  buttonsConfig, closeOnBackdropClick:true, closeOnEsc: true })
...
@Component({
  ...
  template: `
    <button class='btn btn-primary btn-block' nbButton outline status="basic" (click)="openWindowForm()"><i class="fas fa-key"></i></button>
  `,
})
