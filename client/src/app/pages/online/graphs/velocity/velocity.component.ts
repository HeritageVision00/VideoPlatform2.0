...
export class VelocityComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.velocity(this.camera, l).subscribe(
      (res) => {
        this.velocity = res["data"];
        for (var m of this.velocity.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/velocity/' + m['cam_id'] + '/' + m['movie']);
            ...
            datasets: [
              {
                ...
                data: Object.values(this.velocity.over),
