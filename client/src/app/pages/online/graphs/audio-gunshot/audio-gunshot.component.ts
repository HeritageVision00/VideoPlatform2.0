...
@Component({
  selector: 'ngx-audio-gunshot',
  templateUrl: './audio-gunshot.component.html',
  styleUrls: ['./audio-gunshot.component.scss']
})
export class AudioGunshotComponent implements OnInit {
  ...
  rel(event){
    this.cameraSel.emit(event.id);
  }

  ngOnInit(): void {
    this.face.getLiveMics().subscribe(
      res =>{
        this.cameras = res['data']
        if(this.camera !== ''){
          for(const mic of this.cameras){
            if(this.camera === mic.id){
              this.mic = mic
            }
          }
        }
