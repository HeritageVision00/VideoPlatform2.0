...
export class CrowdComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.crowd(this.camera, l).subscribe(
      (res) => {
        this.crowd = res["data"];
        for (var m of this.crowd.raw) {
        ...
        let labels = [];
        for (var o of Object.keys(this.crowd.over)) {
          o = o + ":00:00";
            ...
            datasets: [
              {
                ...
                data: Object.values(this.crowd.over),
  ...
  async csv(algo){
    ...
    ;(await this.serv.report1(this.algoId, this.camera, l)).subscribe(
      async (res) => {
        ...
        data = res['data']
        for (let m of data) {
          m['picture'] = 'http://localhost' + api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/crowd/' + m['cam_id'] + '/' + m['picture']
