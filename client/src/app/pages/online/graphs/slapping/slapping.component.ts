...
@Component({
  selector: "ngx-slapping",
  templateUrl: "./slapping.component.html",
  styleUrls: ["./slapping.component.scss"],
})
export class SlappingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.slapping(this.camera, l).subscribe(
      (res) => {
        this.slapping = res["data"];
        for (var m of this.slapping.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/slapping/' + m['cam_id'] + '/' + m['movie']);
        ...
        this.source = this.slapping.raw
          .slice()
          .sort((a, b) => +new Date(b.time) - +new Date(a.time));
        let labels = [];
        for (var o of Object.keys(this.slapping.over)) {
          o = o + ":00:00";
            ...
            datasets: [
              {
                ...
                data: Object.values(this.slapping.over),
