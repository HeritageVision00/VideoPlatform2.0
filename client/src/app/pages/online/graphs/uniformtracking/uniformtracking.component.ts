...
@Component({
  selector: 'ngx-uniformtracking',
  templateUrl: './uniformtracking.component.html',
  styleUrls: ['./uniformtracking.component.scss']
})
export class UniformtrackingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.serv.uniformtracking(this.camera, l).subscribe(
      res => {
        this.uniformtracking = res['data'];
        let id = 0
        for(const m of this.uniformtracking.raw){
          m['clip_path']  = api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/uniformtracking/' + m['cam_id'] + '/' + m['clip_path'];
          m['picture']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/uniformtracking/' + m['cam_id'] + '/' + m['picture']);
          m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/uniformtracking/' + m['cam_id'] + '/' + m['movie']);
          // m['time'] = new Date(m['time'])
          m['time'] = this.datepipe.transform(m['time'], 'yyyy-M-dd HH:mm:ss',this.timezone);
          m['idTable'] = id
          id++
        }
        this.source = this.uniformtracking.raw.slice().sort((a, b) => +new Date(b.time) - +new Date(a.time));
        if (Object.keys(this.uniformtracking.histogram).length !== 0){
          const labels = [];
          for (const o of Object.keys(this.uniformtracking.histogram)){
            labels.push(JSON.stringify(parseInt(o) + parseInt(this.timezone)) + ' hrs');
          }
          const times = [];
          for (const q of this.uniformtracking.labelsD){
          ...
          this.themeSubscription = this.theme.getJsTheme().subscribe(config => {
            ...
            this.dataH = {
              labels: labels,
              datasets: [{
                  label: 'uniformtracking',
                  backgroundColor: colors.infoLight,
                  borderWidth: 1,
                  data: Object.values(this.uniformtracking.histogram),
                },
              ],
            };
            ...
            this.dataL = {
              ...
              datasets: [{
                label: 'uniformtracking',
                data: this.uniformtracking.dwell,
