...
@Component({
  selector: 'ngx-abduction',
  templateUrl: './abduction.component.html',
  styleUrls: ['./abduction.component.scss']
})
export class AbductionComponent implements OnInit , OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.abduction(this.camera, l).subscribe(
      (res) => {
        ...
        this.source = this.abduction.raw.slice().sort((a, b) => +new Date(b.time) - +new Date(a.time));
        let labels = [];
        for (var o of Object.keys(this.abduction.over)) {
          o = o + ":00:00";
