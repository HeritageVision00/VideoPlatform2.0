...
export class FrComponent implements OnInit , OnDestroy {
    ...
    this.serv.fr(this.camera, l).subscribe(
      res => {
        ...
        for (const m of this.fr.raw) {
          m['picture'] = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/black_white_fr/' + m['cam_id'] + '/' + m['picture']);
  ...
  async csv(algo){
    ...
    ;(await this.serv.report1(0, this.camera, l)).subscribe(
      async (res) => {
        let data: Array<any>;
        data = res['data']
        for (let m of data) {
          m['picture'] = 'http://localhost' + api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/black_white_fr/' + m['cam_id'] + '/' + m['picture']
          m['time'] = this.datepipe.transform(m['time'], 'yyyy-M-dd HH:mm:ss', this.timezone);
        }
