...
export class DefectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.defect(this.camera, l).subscribe(
      (res) => {
        this.defect = res["data"];
        for (var m of this.defect.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/defect/' + m['cam_id'] + '/' + m['movie']);
        ...
        this.source = this.defect.raw.slice().sort((a, b) => +new Date(b.time) - +new Date(a.time));

        let labels = [];
        for (var o of Object.keys(this.defect.over)) {
          o = o + ":00:00";
            ...
            datasets: [
              {
                ...
                data: Object.values(this.defect.over),
