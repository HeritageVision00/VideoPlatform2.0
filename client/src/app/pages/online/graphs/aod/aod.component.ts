      ...
      (res) => {
        ...
        for (var m of this.aod.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/aod/' + m['cam_id'] + '/' + m['movie']);
  ...
  async csv(algo){
    ...
    ;(await this.serv.report1(this.algoId, this.camera, l)).subscribe(
      async (res) => {
        ...
        data = res['data']
        for (let m of data) {
          m['picture'] = 'http://localhost' + api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/aod/' + m['cam_id'] + '/' + m['picture']
          m['time'] = this.datepipe.transform(m['time'], 'yyyy-M-dd HH:mm:ss', this.timezone);
        }
