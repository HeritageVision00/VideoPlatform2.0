...
export class AnprComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.tz = JSON.parse(localStorage.getItem("info")).timezone
    ...
    this.serv.anpr(this.camera, l).subscribe(
      res => {
        ...
        for (const m of this.anpr.raw) {
          ...
          m['time'] = this.datepipe.transform(m['time'], 'yyyy-M-dd HH:mm:ss', this.tz);
          m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/anpr/' + m['cam_id'] + '/' + m['movie']);
        }
  ...
  settings = {
    ...
    columns: {
      ...
      //   type: 'string',
      //   filter: false,
      //   // valuePrepareFunction: (checkout) => { 
      //   //   return this.datepipe.transform(new Date(checkout), 'yyyy-M-dd HH:mm', this.tz);
      //   // }
      // },
      // checkin: {
      //   title: 'CHECKIN',
      //   type: 'string',
      //   filter: false,
      //   // valuePrepareFunction: (checkin) => { 
      //   //   return this.datepipe.transform(new Date(checkin), 'yyyy-M-dd HH:mm', this.tz);
      //   // }
      // },
      dwelltime: {
        title: 'DWELL',
