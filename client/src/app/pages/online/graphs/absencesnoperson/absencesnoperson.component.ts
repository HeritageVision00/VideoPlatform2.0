...
@Component({
  selector: 'ngx-absencesnoperson',
  templateUrl: './absencesnoperson.component.html',
  styleUrls: ['./absencesnoperson.component.scss']
})
export class AbsencesnopersonComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.face.checkVideo(118, this.camera).subscribe(
      res => {
    ...
    this.serv.compiledRequest(this.camera, l ,118).subscribe(
        res => {
          this.intrude = res['data'];
          for (const m of this.intrude.raw){
            
            m['clip_path']  = api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/absences/' + m['cam_id'] + '/' + m['clip_path'];
            m['picture']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/absences/' + m['cam_id'] + '/' + m['picture']);
            m['time'] = this.datepipe.transform(m['time'], 'yyyy-M-dd HH:mm:ss', this.timezone);
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/absences/' + m['cam_id'] + '/' + m['movie']);
          }
