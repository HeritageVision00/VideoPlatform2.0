...
@Component({
  selector: 'ngx-enter-exit-v',
  templateUrl: './enter-exit-v.component.html',
  styleUrls: ['./enter-exit-v.component.scss']
})
export class EnterExitVComponent implements OnInit  , OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.enterExitV(this.camera, l).subscribe(
      (res) => {
        this.enterExitV = res["data"];
        for (var m of this.enterExitV.raw) {
          ...
          m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/enterExitV/' + m['cam_id'] + '/' + m['movie']);
          m["clip_path"] =
            api +
            "/pictures/" +
            this.now_user["id_account"] +
        ...
        this.source = this.enterExitV.raw.slice().sort((a, b) => +new Date(b.time) - +new Date(a.time));
        let labels = [];
        for (var o of Object.keys(this.enterExitV.over)) {
          o = o + ":00:00";
          // labels.push(o)
            ...
            datasets: [
              {
                ...
                data: Object.values(this.enterExitV.over),
