...
@Component({
  ...
  templateUrl: './no-exit.component.html',
  styleUrls: ['./no-exit.component.scss']
})
export class NoExitComponent implements OnInit , OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.exit(this.camera, l).subscribe(
      (res) => {
        this.exit = res["data"];
        for (var m of this.exit.raw) {
          ...
          m["clip_path"] =
            ...
            "/" +
            m["clip_path"];
            m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/exit/' + m['cam_id'] + '/' + m['movie']);
        ...
        let labels = [];
        for (var o of Object.keys(this.exit.over)) {
          o = o + ":00:00";
            ...
            datasets: [
              {
                ...
                data: Object.values(this.exit.over),
