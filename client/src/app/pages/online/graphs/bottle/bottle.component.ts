...
export class BottleComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.serv.bottle(this.camera, l).subscribe(
      (res) => {
        this.bottle = res["data"];
        for (var m of this.bottle.raw) {
          ...
          m['videoClip']  = this.sanitizer.bypassSecurityTrustUrl(api + '/pictures/' + this.now_user['id_account'] + '/' + m['id_branch'] + '/bottle/' + m['cam_id'] + '/' + m['movie']);
          m["clip_path"] =
            api +
            "/pictures/" +
            this.now_user["id_account"] +
        ...
        // this.source = this.bottle.raw
        //   .slice()
        //   .sort((a, b) => +new Date(b.time) - +new Date(a.time));
        let labels = [];
        for (var o of Object.keys(this.bottle.over)) {
          o = o + ":00:00";
            ...
            datasets: [
              {
                ...
                data: Object.values(this.bottle.over),
