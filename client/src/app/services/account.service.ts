...
API_URI = api;
  ...
  tickets(some: any) {
    ...
    if(typeof some.algo_id === 'number'){
      extra = `&algo_id=${some.algo_id}`
    }
    return new ServerDataSource(this.http, {
      endPoint: `${this.API_URI}/ticket/all?type=${some.type}&id=${some.id}&_sort=createdAt&_order=${order}&timerange=${some.timerange}&date=${some.date}&zone=${some.zone}&cam=${some.cam}${extra}`,
      dataKey: 'data',
      totalKey: 'total',
    });
  }
  ...
  changeTheme(theme: any, id: string) {
    return this.http.put(`${this.API_URI}/auth/changeTheme/${id}`, theme);
  }
  getTheme(id: string) {
    return this.http.get(`${this.API_URI}/auth/getTheme/${id}`);
  }
  love(a:any, time: number){
    if(time){
      return this.http.post(`${this.API_URI}/ticket/all/love?random=${time}`,a);
    } else {
      return this.http.post(`${this.API_URI}/ticket/all/love`,a);
    }
  }
}
