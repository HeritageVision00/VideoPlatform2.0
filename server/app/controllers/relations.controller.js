    ...
    User.findOne({
      ...
      Relation.findAll({
        ...
        .then(async rels => {
          ...
          let atr = cam.dataValues.atributes
          if(atr === null){
            atr = {}
          }
          atr['vsEnd'] = inf[12]['end']
          atr['vsStr'] = inf[12]['beg']
          await Cameras.update({
            atributes: atr
          },{
            where: {id: id}
          })
...
exports.dashboards = (req, res) => {
  let audio = false
  User.findOne({
    where: {id: req.id_account}
  })
    .then(user => {
      var authorities = []
      user.getAlgorithms().then(async roles => {
        ...
        Algorithm.findAll()
          .then(algos => {
            ...
            if (user.id_branch != 0000) {
              wh = {id_branch: req.id_branch}
            } else {
              wh = {id_account: req.id_account}
            }
            Relation.findAll({
              where: wh
            })
              .then(async rels => {
                ...
                Cameras.findAll({
                  ...
                  .then(cameras => {
                    ...
                    analyticsPrem.push({
                      algo_id: -6,
                      name: "Attendance Management Congregated",
                      status: "'primary'"
                    })
                    }
                    let set = {
  ...
  Relation.findOne({
    ...
    .then(rel => {
      ...
      if (rel.http_out === undefined || rel.http_out === null || rel.http_out === '') {
        ...
        // if(http_out.includes(":8090")){
        //   if(process.env.NODE_ENV === 'production'){
        //     http_out = `/apiRoute${http_out.split(":8090")[1]}`
        //   }else {
        //     http_out = http_out.split(":8090").join('/apiRoute')
        //   }
        // }
