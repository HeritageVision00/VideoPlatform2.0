const controller = require('../controllers/analytics.controller')
const { authJwt } = require('../middleware')

module.exports = function (app) {
  app.use(function (req, res, next) {
    res.header('Access-Control-Allow-Headers', 'x-access-token, Origin, Content-Type, Accept')
    next()
  })
  /**
   * @swagger
   *tags:
   *  name: Analytics
   *  description: API to manage the analytics returns.
   */
  /**
   * @swagger
   *tags:
   *  name: Overview Dashboard
   *  description: API to manage the overview dashboard results.
   */

/**
   * @swagger
   * /api/analytics/1/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Climbing Barricade analytic's results"
   *    operationId: climbingBarricade
   *    description: "Use to obtain results from Climbing Barricade algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   avgH:
   *                     description: "Avarage in hours"
   *                     type: number
   *                     example: 32.1
   *                   avgS:
   *                     description: "Avarage in seconds"
   *                     type: string
   *                     example: 12.1
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   dwell:
   *                     description: "Array with the values of dwell"
   *                     type: array
   *                     example: []
   *                   labelsD:
   *                     description: "Labels for the detection times in array format"
   *                     type: array
   *                     example: []
   *                   histogram:
   *                     description: "Counts for the histogram"
   *                     type: object
   *                     example: {}
   *                   min:
   *                     description: "Minimun value for the time variation"
   *                     type: number
   *                     example: 10
   *                   max:
   *                     description: "Maximun value for the time variation"
   *                     type: number
   *                     example: 12
   *                   avg:
   *                     description: "Avarage value for the time variation"
   *                     type: number
   *                     example: 11.5
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */

  app.post('/api/analytics/1/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.climbingBarricade)

  /**
   * @swagger
   * /api/analytics/2/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Loitering analytic's results"
   *    operationId: loitering
   *    description: "Use to obtain results from Loitering algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   avgH:
   *                     description: "Avarage in hours"
   *                     type: number
   *                     example: 32.1
   *                   avgS:
   *                     description: "Avarage in seconds"
   *                     type: string
   *                     example: 12.1
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   dwell:
   *                     description: "Array with the values of dwell"
   *                     type: array
   *                     example: []
   *                   labelsD:
   *                     description: "Labels for the detection times in array format"
   *                     type: array
   *                     example: []
   *                   histogram:
   *                     description: "Counts for the histogram"
   *                     type: object
   *                     example: {}
   *                   min:
   *                     description: "Minimun value for the time variation"
   *                     type: number
   *                     example: 10
   *                   max:
   *                     description: "Maximun value for the time variation"
   *                     type: number
   *                     example: 12
   *                   avg:
   *                     description: "Avarage value for the time variation"
   *                     type: number
   *                     example: 11.5
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/2/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.loitering)

  app.get('/api/analytics/loitering/alerts/:id', [authJwt.verifyToken], controller.loiteringAlerts)

  /**
   * @swagger
   * /api/analytics/17/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Intrusion Alert results"
   *    operationId: intrude
   *    description: "Use to obtain results from Intrusion Alert algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   avgH:
   *                     description: "Avarage in hours"
   *                     type: number
   *                     example: 32.1
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   donut:
   *                     description: "Array with the values for the pie chart"
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/17/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.intrude)

  /**
   * @swagger
   * /api/analytics/19/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Violence algorithm's results"
   *    operationId: violence
   *    description: "Use to obtain results from Violence algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/19/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.violence)

  /**
   * @swagger
   * /api/analytics/16/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Abandoned Object analytic's results"
   *    operationId: aod
   *    description: "Use to obtain results from Abandoned Object algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/16/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.aod)
  /**
   * @swagger
   * /api/analytics/20/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "No Mask analytic's results"
   *    operationId: covered
   *    description: "Use to obtain results from No Mask algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/20/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.covered)

  /**
   * @swagger
   * /api/analytics/21/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Social Distance analytic's results"
   *    operationId: social
   *    description: "Use to obtain results from Social Distance algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   types:
   *                     description: "Object of the result level count."
   *                     type: object
   *                     example: {}
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/21/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.social)

  /**
   * @swagger
   * /api/analytics/12/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "People Count analytic's results"
   *    operationId: pc
   *    description: "Use to obtain results from People Count algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   totalEn:
   *                     description: "Total entrance alerts"
   *                     type: number
   *                     example: 156
   *                   totalEx:
   *                     description: "Total exit alerts"
   *                     type: number
   *                     example: 156
   *                   in:
   *                     description: "Total people inside"
   *                     type: number
   *                     example: 156
   *                   avg:
   *                     description: "Avarage values for people inside, getting out and in."
   *                     type: object
   *                     properties:
   *                       in:
   *                         type: number
   *                         example: 1
   *                       en:
   *                         type: number
   *                         example: 11
   *                       ex:
   *                         type: number
   *                         example: 10
   *                   min:
   *                     description: "Minimun values for people inside, getting out and in."
   *                     type: object
   *                     properties:
   *                       in:
   *                         type: number
   *                         example: 1
   *                       en:
   *                         type: number
   *                         example: 11
   *                       ex:
   *                         type: number
   *                         example: 10
   *                   max:
   *                     description: "Maximun values for people inside, getting out and in."
   *                     type: object
   *                     properties:
   *                       in:
   *                         type: number
   *                         example: 1
   *                       en:
   *                         type: number
   *                         example: 11
   *                       ex:
   *                         type: number
   *                         example: 10
   *                   histogramEn:
   *                     description: "Object of the histogram entrance count."
   *                     type: object
   *                     example: {}
   *                   histogramEx:
   *                     description: "Object of the histogram exit count."
   *                     type: object
   *                     example: {}
   *                   label:
   *                     description: "Labels for graphs."
   *                     type: object
   *                     example: {}
   *                   dataIn:
   *                     description: "Data for graphs, inside people."
   *                     type: array
   *                     example: []
   *                   dataEn:
   *                     description: "Data for graphs, entry people."
   *                     type: array
   *                     example: []
   *                   dataEx:
   *                     description: "Data for graphs, exiting people"
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/12/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.pc)

  /**
   * @swagger
   * /api/analytics/6/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Helmet Detection analytic's results"
   *    operationId: helm
   *    description: "Use to obtain results from Helmet Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/6/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.helm)

  /**
   * @swagger
   * /api/analytics/22/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Queue Management analytic's results"
   *    operationId: queue
   *    description: "Use to obtain results from Queue Management algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   count:
   *                     description: "Count queueing"
   *                     type: number
   *                     example: 12
   *                   avg:
   *                     description: "Avarage time of wait."
   *                     type: number
   *                     example: 00:01:02
   *                   min:
   *                     description: "Minimun time of wait."
   *                     type: number
   *                     example: 00:00:30
   *                   max:
   *                     description: "Maximun time of wait."
   *                     type: number
   *                     example: 00:10:02
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/22/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.queue)

  /**
   * @swagger
   * /api/analytics/24/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Open Vault Door analytic's results"
   *    operationId: vault
   *    description: "Use to obtain results from Open Vault Door algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total number of alerts."
   *                     type: number
   *                     example: 12
   *                   avg:
   *                     description: "Avarage alerts per day."
   *                     type: number
   *                     example: 00:01:02
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   org:
   *                     description: "Values with the duration of the queue."
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/24/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.vault)

  /**
   * @swagger
   * /api/analytics/4/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Parking Violation analytic's results"
   *    operationId: parking
   *    description: "Use to obtain results from Parking Violation algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/4/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.parking)

  /**
   * @swagger
   * /api/analytics/13/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Automatic Number Plate Recognition (ANPR) analytic's results"
   *    operationId: anpr
   *    description: "Use to obtain results from Automatic Number Plate Recognition (ANPR) algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/13/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.anpr)

  /**
   * @swagger
   * /api/analytics/25/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Barrier Detection analytic's results"
   *    operationId: barrier
   *    description: "Use to obtain results from Barrier Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/25/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.barrier)

  /**
   * @swagger
   * /api/analytics/26/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Vehicle Count analytic's results"
   *    operationId: vehicle
   *    description: "Use to obtain results from Vehicle Count algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   totalEn:
   *                     description: "Total entrance alerts"
   *                     type: number
   *                     example: 156
   *                   totalEx:
   *                     description: "Total exit alerts"
   *                     type: number
   *                     example: 156
   *                   in:
   *                     description: "Total vehicles inside"
   *                     type: number
   *                     example: 156
   *                   avg:
   *                     description: "Avarage values for vehicles inside, getting out and in."
   *                     type: object
   *                     properties:
   *                       in:
   *                         type: number
   *                         example: 1
   *                       en:
   *                         type: number
   *                         example: 11
   *                       ex:
   *                         type: number
   *                         example: 10
   *                   min:
   *                     description: "Minimun values for vehicles inside, getting out and in."
   *                     type: object
   *                     properties:
   *                       in:
   *                         type: number
   *                         example: 1
   *                       en:
   *                         type: number
   *                         example: 11
   *                       ex:
   *                         type: number
   *                         example: 10
   *                   max:
   *                     description: "Maximun values for vehicles inside, getting out and in."
   *                     type: object
   *                     properties:
   *                       in:
   *                         type: number
   *                         example: 1
   *                       en:
   *                         type: number
   *                         example: 11
   *                       ex:
   *                         type: number
   *                         example: 10
   *                   histogramEn:
   *                     description: "Object of the histogram entrance count."
   *                     type: object
   *                     example: {}
   *                   histogramEx:
   *                     description: "Object of the histogram exit count."
   *                     type: object
   *                     example: {}
   *                   label:
   *                     description: "Labels for graphs."
   *                     type: object
   *                     example: {}
   *                   dataIn:
   *                     description: "Data for graphs, inside vehicles."
   *                     type: array
   *                     example: []
   *                   dataEn:
   *                     description: "Data for graphs, entry vehicles."
   *                     type: array
   *                     example: []
   *                   dataEx:
   *                     description: "Data for graphs, exiting vehicles"
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/26/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.vc)

  /**
   * @swagger
   * /api/analytics/queueLite/{id}:
   *   post:
   *    tags:
   *       - Overview Dashboard
   *    summary: "Queue Managemente Lite results"
   *    operationId: queueLite
   *    description: "Use to obtain results from Queue Managemente algorithm for an specific camera under a range of time. This is the lite version to load main Dashboard overview."
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   avg:
   *                     description: "Avarage time in queue."
   *                     type: number
   *                     example: 00:01:03
   *                   count:
   *                     description: "Quantity of people in queue."
   *                     type: number
   *                     example: 12
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/queueLite/:id', [authJwt.verifyToken], controller.queueLite)

  /**
   * @swagger
   * /api/analytics/pcLite/{id}:
   *   post:
   *    tags:
   *       - Overview Dashboard
   *    summary: "People Count lite results"
   *    operationId: pcLite
   *    description: "Use to obtain results from People Count algorithm for an specific camera under a range of time. This is the lite version for Overview Dashboard."
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   currentCount:
   *                     description: "Current count of people."
   *                     type: number
   *                     example: 13
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/pcLite/:id', [authJwt.verifyToken], controller.pcLite)

  /**
   * @swagger
   * /api/analytics/premises/{id}:
   *   post:
   *    tags:
   *       - Overview Dashboard
   *    summary: "Premises overview results"
   *    operationId: premises
   *    description: "Use to obtain results from Premises for an specific camera under a range of time. This is for the main overview Dashboard."
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   queue:
   *                     description: "Data for queue graph."
   *                     type: object
   *                     example: {}
   *                   pcount:
   *                     description: "Data for people count graph."
   *                     type: object
   *                     example: {}
   *                   loit:
   *                     description: "Data for loitering graph."
   *                     type: object
   *                     example: {}
   *                   vault:
   *                     description: "Data for vault graph."
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/premises/:id', [authJwt.verifyToken], controller.premises)

  /**
   * @swagger
   * /api/analytics/threats/{id}:
   *   post:
   *    tags:
   *       - Overview Dashboard
   *    summary: "Threats overview results"
   *    operationId: threats
   *    description: "Use to obtain results from Threats for an specific camera under a range of time. This is for the main overview Dashboard."
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   violence:
   *                     description: "Data for violence graph."
   *                     type: object
   *                     example: {}
   *                   helmet:
   *                     description: "Data for helmet graph."
   *                     type: object
   *                     example: {}
   *                   covered:
   *                     description: "Data for no-mask graph."
   *                     type: object
   *                     example: {}
   *                   aod:
   *                     description: "Data for abandoned object graph."
   *                     type: object
   *                     example: {}
   *                   social:
   *                     description: "Data for social distancing graph."
   *                     type: object
   *                     example: {}
   *                   intr:
   *                     description: "Data for intrusion graph."
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/threats/:id', [authJwt.verifyToken], controller.threats)
  /**
   * @swagger
   * /api/analytics/27/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Camera Tamper analytic's results"
   *    operationId: tamper
   *    description: "Use to obtain results from Camera Tamper algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   over:
   *                     description: "Over time data graph"
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/27/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.tamper)

  /**
   * @swagger
   * /api/analytics/29/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Accident Detection analytic's results"
   *    operationId: accident
   *    description: "Use to obtain results from Accident Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/29/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.accident)
  /**
   * @swagger
   * /api/analytics/28/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Animals On Road analytic's results"
   *    operationId: animal
   *    description: "Use to obtain results from Animals On Road algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/28/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.animal)
  /**
   * @swagger
   * /api/analytics/30/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Axle Detection analytic's results"
   *    operationId: axle
   *    description: "Use to obtain results from Axle Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/30/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.axle)
  /**
   * @swagger
   * /api/analytics/31/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Axle Detection analytic's results"
   *    operationId: carmake
   *    description: "Use to obtain results from Carmake Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/31/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.carmake)
  /**
   * @swagger
   * /api/analytics/33/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Vehicle Count analytic's results"
   *    operationId: vcount
   *    description: "Use to obtain results from Vehicle Count algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/33/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.vcount)
  /**
   * @swagger
   * /api/analytics/8/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Wrong way or illegal turn detection analytic's results"
   *    operationId: direction
   *    description: "Use to obtain results from Wrong way or illegal turn detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/8/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.direction)
  /**
   * @swagger
   * /api/analytics/5/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Speeding Vehicle analytic's results"
   *    operationId: speeding
   *    description: "Use to obtain results from Speeding Vehicle algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   rel:
   *                     description: "Raw data from relations containing rtsp link."
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/5/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.speeding)
  /**
   * @swagger
   * /api/analytics/0/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Demographics analytic's results"
   *    operationId: fr
   *    description: "Use to obtain results from Demographics algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   rel:
   *                     description: "Raw data from relations containing rtsp link."
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */

  app.post('/api/analytics/0/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.fr)

  /**
   * @swagger
   * /api/analytics/32/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Clothing analytic's results"
   *    operationId: clothing
   *    description: "Use to obtain results from Clothing algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *                   rel:
   *                     description: "Raw data from relations containing rtsp link."
   *                     type: object
   *                     example: {}
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */

  app.post('/api/analytics/32/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.cloth)
  /**
   * @swagger
   * /api/analytics/34/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Car Brand Detection analytic's results"
   *    operationId: brand
   *    description: "Use to obtain results from Car Brand Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/34/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.brand)

  app.post('/api/analytics/pcOnView/:id', [authJwt.verifyToken], controller.pcOnView)

  /**
   * @swagger
   * /{id}:
   *   get:
   *    tags:
   *       - Analytics
   *    summary: "Current Audience Profile"
   *    operationId: audience
   *    description: "Use to obtain results of the current audience."
   *    parameters:
   *      - in: path
   *        name: id camera
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               no_of_persons:
   *                 description: "Count of persons"
   *                 type: number
   *                 example: 2
   *               audience:
   *                 type: array
   *                 example: [{ age: 'adult', gender: 'female', emotion: 'neutral', headPose: 'Away' },{ age: 'adult', gender: 'male', emotion: 'happy', headPose: 'Bottom Left' }]
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */

  /**
   * @swagger
   * /api/analytics/39/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Fire Detection analytic's results"
   *    operationId: brand
   *    description: "Use to obtain results from Fire Detection algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/39/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.fire)

  /**
   * @swagger
   * /api/analytics/38/{id}:
   *   post:
   *    tags:
   *       - Analytics
   *    summary: "Person Collapsing analytic's results"
   *    operationId: brand
   *    description: "Use to obtain results from PErson Collapsing algorithm for an specific camera under a range of time"
   *    parameters:
   *      - in: path
   *        name: id
   *        schema:
   *          type: string
   *        required: true
   *      - in: header
   *        name: x-access-token
   *        schema:
   *          type: string
   *        required: true
   *    requestBody:
   *       content:
   *          application/json:
   *            schema:
   *              type: object
   *              properties:
   *                type:
   *                  description: "Type of data to be retrieved. Can be cam_id, id_branch or id_account."
   *                  type: string
   *                  example: cam_id
   *                start:
   *                  description: "Date for initial range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-10-01T03:00:00.000Z
   *                end:
   *                  description: "Date for end range. Format: 2020-10-01T03:00:00.000Z."
   *                  type: string
   *                  example: 2020-11-01T02:59:59.000Z
   *    responses:
   *      '200':
   *        description: "A successful response"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: true
   *               data:
   *                 type: object
   *                 properties:
   *                   total:
   *                     description: "Total alerts"
   *                     type: number
   *                     example: 156
   *                   raw:
   *                     description: "Raw data from MySQL."
   *                     type: array
   *                     example: []
   *      '500':
   *        description: "Server error"
   *        content:
   *          application/json:
   *            schema:
   *             type: object
   *             properties:
   *               success:
   *                 description: "Result type"
   *                 type: boolean
   *                 example: false
   *               message:
   *                 description: "Type of failure"
   *                 type: string
   *                 example: Error message generated by the server
   */
  app.post('/api/analytics/38/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.collapse)

  app.post('/api/analytics/35/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.weapon)

  app.post('/api/analytics/36/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.bottle)

  app.post('/api/analytics/41/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.waving)

  app.post('/api/analytics/42/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.smoking)

  app.post('/api/analytics/44/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.slapping)

  app.post('/api/analytics/46/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.running)

  app.post('/api/analytics/50/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.pushing)

  app.post('/api/analytics/48/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.purse)

  app.post('/api/analytics/40/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.hair)

  app.post('/api/analytics/49/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.following)

  app.post('/api/analytics/47/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.disrobing)

  app.post('/api/analytics/43/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.crowd)

  app.post('/api/analytics/45/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.blocking)

  app.post('/api/analytics/52/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.transpassing)

  app.post('/api/analytics/53/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.cameraDefocused)

  app.post('/api/analytics/55/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.sceneChange)

  app.post('/api/analytics/54/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.cameraBlinded)

  app.post('/api/analytics/56/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.objectRemoval)

  app.post('/api/analytics/57/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.smokeDetection)

  app.post('/api/analytics/58/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.velocity)

  app.post('/api/analytics/60/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.exit)

  app.post('/api/analytics/59/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.enterExit)

  app.post('/api/analytics/61/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.harrasment)

  app.post('/api/analytics/62/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.abduction)

  app.post('/api/analytics/63/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.direction)

  app.post('/api/analytics/64/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.signalLost)

  app.post('/api/analytics/65/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.enterExitV)

  app.post('/api/analytics/70/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.congestion)

  app.post('/api/analytics/72/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.vehloitering)

  app.post('/api/analytics/71/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.ppe)

  app.post('/api/analytics/73/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.defect)

  app.post('/api/analytics/74/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.module)

  app.post('/api/analytics/51/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.peopleTracking)

  app.post('/api/analytics/9/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.graffiti)

  app.post('/api/analytics/18/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.am)

  app.post('/api/analytics/75/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.trafficSignal)

  app.post('/api/analytics/76/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.flashingNudity)

  app.post('/api/analytics/77/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.stalkingWomanArea)

  app.post('/api/analytics/78/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.stalkingWomanNight)

  app.post('/api/analytics/79/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.stalkingWomanScooty)

  app.post('/api/analytics/80/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.audioGunshot)

  app.post('/api/analytics/81/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.InjuryAndBlood)

  app.post('/api/analytics/82/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.SuicideTendency)

  app.post('/api/analytics/83/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.SmokersGroup)

  app.post('/api/analytics/84/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.DrinkersGroup)

  app.post('/api/analytics/85/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.TeasingWoman)

  app.post('/api/analytics/86/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.OpenDefecation)

  app.post('/api/analytics/87/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.acidAttack)

  app.post('/api/analytics/88/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.zigzag)

  app.post('/api/analytics/89/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.malemovement)

  app.post('/api/analytics/90/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.malebehaviour)

  app.post('/api/analytics/91/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.AbnormalBehavior)

  app.post('/api/analytics/92/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.jaywalking)

  app.post('/api/analytics/93/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.AbnormalRunning)

  app.post('/api/analytics/94/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.GamblingSpot)

  app.post('/api/analytics/95/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.FrequentVisiting)

  app.post('/api/analytics/96/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.StrandedGirl)

  app.post('/api/analytics/97/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.UnconsciousWoman)

  app.post('/api/analytics/98/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.StuntBikers)

  app.post('/api/analytics/99/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.InkThrowing)

  app.post('/api/analytics/100/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.DrugsDistribution)

  app.post('/api/analytics/101/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.PornographicMaterial)

  app.post('/api/analytics/102/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.Kidnapping)

  app.post('/api/analytics/103/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.WomenSafety)

  app.post('/api/analytics/104/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.WomenCollapse)

  app.post('/api/analytics/105/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.WomenViolence)

  app.post('/api/analytics/106/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.SlappingWomen)

  app.post('/api/analytics/107/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.AnprSnatching)

  app.post('/api/analytics/108/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.MenAtShadowArea)

  app.post('/api/analytics/109/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.FreqVisiting)

  app.post('/api/analytics/110/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.uniformtracking)

  app.post('/api/analytics/111/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.edgeCross)

  app.post('/api/analytics/112/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.jumpTrack)

  app.post('/api/analytics/115/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.streetVendor)

  app.post('/api/analytics/116/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.nurseDwell)

  app.post('/api/analytics/117/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.bagdetected)

  app.post('/api/analytics/118/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.absences)

  app.post('/api/analytics/119/:id', [authJwt.isAvailable, authJwt.verifyToken], controller.bycicleMotor)
}
